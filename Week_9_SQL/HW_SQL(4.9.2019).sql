...
select last_name, count(last_name) as 'last_name_frequency' from actor
GROUP BY last_name
having `last_name_frequency`;
...
SUM(payment.amount) AS 'Total Amount Paid' FROM payment
...
SELECT store.store_id, SUM(amount) AS Gross FROM payment
...
SELECT SUM(amount) AS 'Total Sales', category.name AS 'Genre' FROM payment
INNER JOIN rental ON (payment.rental_id = rental.rental_id)
